...
def extract_product_info(text):
    ...
    for ent in doc.ents:
        if ent.label_ == "PRODUCT":
            info.setdefault('product_name', []).append(ent.text)
        elif ent.label_ == "MONEY":
            info.setdefault('price', []).append((float(ent.text.replace('$', '')), 'usd'))

    # Use Hugging Face NER for additional entity recognition
    ner_results = ner_pipeline(text)
    for result in ner_results:
        if result['entity'] == 'MISC' and len(result['word']) > 3:
            info.setdefault('product_name', []).append(result['word'])
